...
class StatementManager:
    ...
    def instantiate(self, class_name, args):
        ...
        # Gets the scope of the class
        class_scope = self.table.table[class_name]
        # Creates a list of all the variable symbols in the class scope
        ...
        for (name,symbol) in class_attributes:
            symbol_attributes[name] = self.memory.get_address('instance', symbol.type)
    ...
    def assign(self, variable, value):
        ...
        if len(value) == 3 and isinstance(value[3], VariableSymbol):
            new_symbol = copy.deepcopy(value[3])
            new_attrs = self.copy_attributes(value[3]) 
            new_symbol.address = variable[0]
            for name,attr in new_symbol.attrs:
                new_address = self.memory.get_address(self.current_scope, class_symbols[name].type)
                self.quads.generate(OpIds.assign, attr, 0, new_address)
            self.table.replace_symbol(variable[3], new_symbol)
        self.quads.generate(OpIds.assign, value[0], 0, variable[0])
        return variable
                        
    def copy_attibutes(self):
        # If it's an object instance
        if value[2] not in ['int', 'float', 'bool', 'str']:
            # Get symbols of the variable's class
            class_symbols = self.table.table[value[2]].symbols 
